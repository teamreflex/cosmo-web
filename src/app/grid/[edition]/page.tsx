...
const getEdition = cache(async (edition: string) => {
  ...
  const [profile, grids] = await Promise.all([
    ...
    fetchEdition(token!.accessToken, artist, edition),
  ]);
  ...
  return { title, profile, grids, token, artist };
});
...
export default async function GridEditionPage(props: Props) {
  const params = await props.params;
  const { title, profile, grids, token, artist } = await getEdition(
    params.edition
  );
  ...
  return (
    <UserStateProvider token={token} artist={artist}>
