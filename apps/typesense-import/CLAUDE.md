...
- Standard Effect patterns: `Effect.Service` classes provided via `Layer.mergeAll`, config through the `Env` service (`Effect.Config`, secrets use `Config.redacted`), promises wrapped in `Effect.tryPromise` with per-failure-mode `Data.TaggedError` classes. Use context7 for Effect API docs.
- Failure handling: each import-loop tick is wrapped in `Effect.catchAllCause` — a transient failure logs its cause, the watermark stays put, and the batch is retried on the next tick. Setup effects have no such wrapper and stay fatal at boot.
