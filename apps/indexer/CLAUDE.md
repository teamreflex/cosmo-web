...
9. **Autovacuum reloptions are mostly live-only:** `objekt`, `transfer`, `collection` and `list_event_outbox` have per-table autovacuum settings applied directly in production (not in any migration) because the default thresholds let the visibility map go stale under the constant owner-rewrite churn; `progress_leaderboard` gets its settings from its own migration. All of them are recorded under "Current VACUUM settings" in `docs/database.md` — re-apply the live-only ones by hand after any from-scratch database rebuild.
10. **Leaderboard triggers:** statement-level triggers on `objekt` and a row trigger on `collection` keep `collection_owner` and `progress_leaderboard` current (migration `1790444346394`), so every objekt write, including hot-block rollbacks, runs through them. For a full reindex from the start block, disable the triggers, reindex, then truncate both tables and re-run the migration's backfill. `collection.unobtainable` is set by hand and `handleCollection` must never assign it; a hot-block rollback restores Subsquid's pre-block row snapshot, so a flag flipped while a block touching that collection is still unfinalized can be reverted.
