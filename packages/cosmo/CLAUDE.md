...
- `src/server/` — Effect functions, imported via `@apollo/cosmo/server/*`. HTTP via the `HttpClient` from `effect/unstable/http`, responses validated with `Schema` at the boundary. Effect apps (`apps/schedules`) compose these directly into their own runtimes.
...
- `fetchGravity` fails like any other function; swallowing the failure is a caller concern (`apps/web` wraps it in `runCosmo(...).catch(() => null)`).
...
`bun test` runs the suite (`tests/`) against a mock `HttpClient` layer (`tests/test-client.ts`, built with `HttpClient.make`) — no fetch monkey-patching, no seams in src needed. Tests register handlers for the real COSMO URLs via `handle.get`/`handle.post` (matched on method + origin + pathname; handlers return web `Response`s and are cleared between tests), record requests with `recorder()`, and run effects through `runTest`, which mirrors `runCosmo`'s runPromiseExit + squash semantics so rejections carry the original tagged errors. Unhandled requests die with a defect naming the method and URL. The suite pins request shapes, response unwrapping, retry counts, error `status` shapes (assert properties, never `instanceof` some HTTP lib's error), and the encryption roundtrips. Trade-off: neither the `FetchHttpClient` transport nor `src/runtime.ts`'s `runCosmo` (runtime layer, signal plumbing, squash) is exercised at runtime — `runTest` replicates the latter, so a behavior change to `runCosmo` itself will not fail this suite; review it manually when touching runtime.ts. The suite tests client policy + functions, which is what it actually pins. If you change client policy or endpoint behavior, update these tests deliberately.
